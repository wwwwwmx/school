      ...
      <div class="dialog-body">
        ...
        <div class="body-border">
           ...
           <div class="second-step" >
            <span style="color:#F64656;float: left;">*</span>&nbsp;<div class="item-left-one">选择文件：</div>
           ...
           <div class="third-step" >
            <div class="img-icon"><img src="../../../../assets/img/admin/choose-file.png">&nbsp;选择</div>
           <input id="inputFile" type="file" accept='.xlsx' ref="files" @change="showName"/></div>
      </div>
    ...
    getTeacherPSD() {
      this.$newApi.excel.getExcelUrl("student").then((res) => {
        if (res.code == 0) {
          ...
          this.$message.error("学生导入模板获取异常", result.message);
        }
      });
    },
 btnCancel() {
     ...
     async bulkImport(){
      ...
      //  this.symbol = +new Date();
        ...
        if(this.filename!='仅支持扩展名为xlsx的文件'){
          const info = await this.$newApi.excel.importStudent(payload,{'classesId':this.classesId},'multipart/form-data')
           if(info.code==0){
